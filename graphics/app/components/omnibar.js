...
define([
	...
	'numeral',
	'tabulate'
], (preloader, globals, Stage, layout, numeral, tabulate) => {
	'use strict';
	...
	const gdqLogo = new createjs.Bitmap(preloader.getResult('omnibar-logo-gdq'));
	const GDQ_LOGO_WIDTH = gdqLogo.getBounds().width;

	const pcfLogo = new createjs.Bitmap(preloader.getResult('omnibar-logo-pcf'));
		...
		if (upNextRun) {
			...
			tl.to({}, 0.01, {
				onComplete() {
					 ...
					 */
					const upNextRun = layout.currentLayoutName === 'break' ? globals.currentRun : globals.nextRun;
					if (upNextRun) {
